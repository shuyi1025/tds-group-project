...
# input data
train_path = "../../pipeline_outputs/aim1_output/ukb_train_baseline.csv"
test_path = "../../pipeline_outputs/aim1_output/ukb_test_baseline.csv"
...
def fit_final_model(x_train_df, y_train, x_test_df, y_test, best_params):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "nn_base_training_loss.png"), dpi=300)
    plt.close()
...
def main():
    ...
    results_df.to_csv(
        output_dir / "nn_base_hyperparameter_search_results.csv",
        index=False
    )

    results_df.head(10).to_csv(
        output_dir / "nn_base_hyperparameter_top10.csv",
        index=False
    )
    ...
    with open(output_dir / "best_nn_params_baseline.json", "w") as f:
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "nn_base_hyperparameter_ranking.png", dpi=300)
    plt.close()
    ...
    metrics_df.to_csv(os.path.join(output_dir, "nn_base_metrics.csv"), index=False)

    cm = confusion_matrix(y_test, preds)
    report = classification_report(y_test, preds, zero_division=0)

    print("\n===== NN Baseline test results =====")
    ...
    result_path = os.path.join(output_dir, "nn_base_results.txt")
    with open(result_path, "w", encoding="utf-8") as f:
        f.write("===== Neural Network Baseline Test Performance =====\n")
    ...
    roc_path = os.path.join(output_dir, "nn_base_curve.png")
    ...
    results.to_csv(os.path.join(output_dir, "nn_base_predictions.csv"), index=False)
    print("nn base predictions saved")
    ...
    df.to_csv(os.path.join(auc_path, "nn_baseline.csv"), index=False)
    print("roc overlay results saved")

    # optional save of model + preprocessor
    torch.save(model.state_dict(), os.path.join(output_dir, "nn_base_model_state_dict.pt"))
