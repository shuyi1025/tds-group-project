...
TRAIN_PATH = "../../pipeline_outputs/aim2_3_output/final_datasets/train_ee_bio_stable_allclusters.csv"  # change if needed
TEST_PATH = "../../pipeline_outputs/aim2_3_output/final_datasets/test_ee_bio_stable_allclusters.csv"    # change if needed
OUTPUT_DIR = Path("../../pipeline_outputs/aim4_output/XGBoost_outputs")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
OUTPUT_TAG = "xgboost_biomarker"  # change this for other datasets
...
def train_one_fold(X_train_df, y_train, X_val_df, y_val, params):
    ...
    if best_iteration is None:
        best_iteration = MAX_N_ESTIMATORS - 1
...
def fit_final_model(X_train_df, y_train, X_test_df, best_params):
    ...
    final_model = XGBClassifier(
        objective="binary:logistic",
        eval_metric="auc",
        n_estimators=int(best_params["chosen_n_estimators"]),
...
def main():
    ...
    for combo_idx, params in enumerate(grid, start=1):
        ...
        for fold, (tr_idx, val_idx) in enumerate(skf.split(X_train_full, y_train_full), start=1):
            ...
            print(
                ...
                f"at boosting round {fold_result['best_iteration'] + 1}"
            )
        ...
        result_row = {
            **params,
            ...
            "mean_best_iteration": int(np.round(np.mean(fold_best_iterations))),
            "min_best_iteration": int(np.min(fold_best_iterations)),
            "max_best_iteration": int(np.max(fold_best_iterations)),
            "mean_best_n_estimators": int(np.round(np.mean(fold_best_iterations))) + 1,
        }
    ...
    best_params = {
        ...
        "reg_alpha": 0.0,
        "chosen_n_estimators": int(best_row["mean_best_n_estimators"]),
    ...
    plt.title("ROC Curve - XGBoost Final Test")
    plt.legend()
    plt.tight_layout()
    ...
    df_auc.to_csv(auc_path / "xgb_baseline_ee_biomarker.csv", index=False)
