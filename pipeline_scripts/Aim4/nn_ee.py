...
# input data
train_path = "../../pipeline_outputs/aim1_output/train_baseline_ee_onehot.csv"
test_path = "../../pipeline_outputs/aim1_output/test_baseline_ee_onehot.csv"
...
def fit_final_model(x_train_df, y_train, x_test_df, y_test, best_params):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "nn_ee_training_loss.png"), dpi=300)
    plt.close()
...
def main():
    ...
    results_df.to_csv(
        output_dir / "nn_ee_hyperparameter_search_results.csv",
        index=False
    )

    results_df.head(10).to_csv(
        output_dir / "nn_ee_hyperparameter_top10.csv",
        index=False
    )
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "nn_ee_hyperparameter_ranking.png", dpi=300)
    plt.close()
    ...
    metrics_df.to_csv(os.path.join(output_dir, "nn_ee_metrics.csv"), index=False)

    cm = confusion_matrix(y_test, preds)
    report = classification_report(y_test, preds, zero_division=0)

    print("\n===== NN Baseline + EE test results =====")
    ...
    result_path = os.path.join(output_dir, "nn_ee_results.txt")
    with open(result_path, "w", encoding="utf-8") as f:
        f.write("===== Neural Network Baseline + EE Test Performance =====\n")
    ...
    roc_path = os.path.join(output_dir, "nn_ee_curve.png")
    ...
    results.to_csv(os.path.join(output_dir, "nn_ee_predictions.csv"), index=False)
    ...
    df.to_csv(os.path.join(auc_path, "nn_baseline_ee.csv"), index=False)
    print("roc overlay results saved")

    # optional save of model + preprocessor
    torch.save(model.state_dict(), os.path.join(output_dir, "nn_ee_model_state_dict.pt"))
